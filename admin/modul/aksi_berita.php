...
window.location = 'login.php';  
  ...
  elseif ($modul == 'aksi_berita' and $act == 'update_berita') { // ini dirubah
    ...
    if (empty($lokasi_file)) {
      mysqli_query($koneksi, "UPDATE berita SET    tgl_berita      = '$_POST[tgl_berita]',
                                judul           = '$_POST[judul]',
                                isi             = '$_POST[isi]',
                                slug='$slug'
    ...
    window.location.href='index.php?modul=berita';
      ...
      //EDIT FOTO
      $datafoto = mysqli_query($koneksi, "SELECT foto FROM berita WHERE id_berita='$_POST[kode]'"); //dirubah
      $r = mysqli_fetch_array($datafoto);
      @unlink('../Gambar/Gambar_berita/' . $r['foto']); //dirubah
      @unlink('../Gambar/Gambar_berita/' . 'small_' . $r['foto']); //dirubah
      UploadImage($nama_file_unik);
      mysqli_query($koneksi, "UPDATE berita 
  ...
  elseif ($modul == 'aksi_berita' and $act == 'hapus_berita') {
    $datafoto = mysqli_query($koneksi, "SELECT foto FROM berita WHERE id_berita='$_GET[id]'"); //dirubah
    $r = mysqli_fetch_array($datafoto);
    @unlink('../Gambar/Gambar_berita/' . $r['foto']); //dirubah
    @unlink('../Gambar/Gambar_berita/' . 'small_' . $r['foto']); //dirubah
    mysqli_query($koneksi, "DELETE FROM berita WHERE id_berita = '$_GET[id]'"); // dirubah
    echo ("<SCRIPT LANGUAGE='JavaScript'>
