...
window.location = 'login.php';  
  ...
  if ($modul == 'aksi_admin' and $act == 'input_admin') {
    ...
    if (!empty($lokasi_file)) {
      ...
      $cek_username = mysqli_num_rows(mysqli_query($koneksi, "SELECT username FROM admin
  ...
  elseif ($modul == 'aksi_admin' and $act == 'update_admin') {
    ...
    window.location.href='index.php?modul=admin';
      ...
      //EDIT FOTO
      $datafoto = mysqli_query($koneksi, "SELECT foto FROM admin WHERE id_admin='$_POST[kode]'"); //dirubah
      $r = mysqli_fetch_array($datafoto);
  ...
  elseif ($modul == 'aksi_admin' and $act == 'hapus_admin') {
    $datafoto = mysqli_query($koneksi, "SELECT foto FROM admin WHERE id_admin='$_GET[id]'"); //dirubah
    $r = mysqli_fetch_array($datafoto);
    @unlink('../Gambar/Gambar_user/' . $r['foto']); //dirubah
    @unlink('../Gambar/Gambar_user/' . 'small_' . $r['foto']); //dirubah
    mysqli_query($koneksi, "DELETE FROM admin WHERE id_admin = '$_GET[id]'"); // dirubah
    echo ("<SCRIPT LANGUAGE='JavaScript'>
  ...
  elseif ($modul == 'aksi_admin' and $act == 'update_admin_password') {
    ...
    mysqli_query($koneksi, "UPDATE admin set password='$password' WHERE id_admin='$id_admin'");
    echo ("<SCRIPT LANGUAGE='JavaScript'>
